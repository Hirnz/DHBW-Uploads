...
from colorama import Fore, Style  # Makes it possible to color the text in the terminal


valid_options = ['title', 'artist', 'album', 'year']        # Valid options for search and sort attributes
valid_search_methods = ['linear', 'binary']                 # Valid options for search algorithms
valid_sort_algorithms = ['insertion', 'selection', 'quick'] # Valid options for sorting algorithms


#####################################################################################
# Functionalities to navigate and design the menu, meaning the terminal as the user sees it
def clear_screen():
    # Clear the terminal screen for better orientation in the actual menu (terminal)
...
def display_menu():
    ...
    print("3. Manage playlists")  # Playlist submenu
    print("4. Manage favorites")  # Favorites submenu
...
def display_playlist_menu():
    ...
    print("5. List all playlists")  # Added option to list all playlists
...
def perform_playlist_action(choice, library):
    if choice == '1':
        ...
        library.list_playlists()  # Now lists all playlists within the submenu
    else:
...
def manage_playlists(library):
    while True:
        ...
        if choice == 'b':
            break  # Exit the playlist management menu
        else:
...
def manage_favorites(library):
    while True:
        ...
        choice = input("Enter your choice: ").lower()
        if choice == 'b':
            break  # Exit the favorites management menu
        else:
...
def perform_action(choice, library):
    if choice == '1':
        ...
        manage_playlists(library)  # Call the playlist management submenu
    elif choice == '4':
        manage_favorites(library)  # Call the favorites management submenu
    elif choice == '5':
        library.search_songs()
    elif choice == '6':
        library.sort_library(None, None)  # Just call the sort function, remove prompts from here
    else:
        print(Fore.RED + "Invalid choice. Please try again." + Style.RESET_ALL)
...
class Library:
    ...
    def load_songs_from_csv(self, file_path):
        try:
            ...
            for _, row in song_df.iterrows():
                # Ensure 'decade' or 'year' is treated as an integer and handle NaN
                year = row['decade'] if pd.notna(row['decade']) else 0
    ...
    def delete_song_from_library(self):
        title = input("Enter the title of the song to delete: ").lower()  # Convert user input to lowercase
    ...
    def add_song_to_playlist(self):
        ...
        title = input("Enter the song title to add: ").lower()  # Convert user input to lowercase
        for playlist in self.playlists:
    ...
    def remove_song_from_playlist(self):
        playlist_name = input("Enter the playlist name: ")
        title = input("Enter the song title to remove: ").lower()  # Convert user input to lowercase
        for playlist in self.playlists:
            if playlist.name == playlist_name:
    ...
    #####################################################################################
    # Searching Algorithm Options:
    # Binary-Search:
    def binary_search(self, search_term, sort_by='title'):
        # Ensure the search term is handled appropriately
        if sort_by == 'year':
            try:
        ...
        # Now that we found one match, expand to find all matches around it
    ...
    def linear_search(self, search_term, search_by='title'):
        ...
        # Handle searching by year as an integer
        if search_by == 'year':
            try:
                search_term = int(search_term)  # Convert search term to an integer for year
            except ValueError:
        ...
        for song in self.songs:
            ...
            # Handle NaN or non-string values for string searches
            if search_by == 'year':
    ...
    #####################################################################################
    # Actual calls to the differen search functions with user interaction and sort function that is needed for binary search:
    # Searching: 
    def search_songs(self):
        # Initialize search_term and results to avoid unbound local errors
        search_term = None
        results = None

        # Keep asking until a valid search option is provided
        while True:
            search_by = input("Search by (title/artist/album/year): ").lower()
            if search_by in valid_options:  # Using generalized valid_options
        ...
        # Keep asking until a valid search method is provided
        while True:
        ...
        elapsed_time = time.time() - start_time  # Calculate elapsed time for the actual search

        # Print search results only once
        if results:
            print(f"{len(results)} song(s) found in {elapsed_time:.6f} seconds:")
            for song in results:
                print(song)  # This will use the __repr__ or __str__ method of the Song class
        else:
            print(f"No matching songs found. Search completed in {elapsed_time:.6f} seconds.")
    #####################################################################################
    # Sorting:
    def sort_library(self, sort_by, algorithm):
        # Validation loop for selecting sorting attribute
        while True:
            sort_by = input("Sort by (title/artist/album/year): ").lower()
            if sort_by in valid_options:  # Using generalized valid_options
                break  # Valid input, exit the loop
            else:
                print("Invalid choice. Please enter one of the following: title, artist, album, or year.")
        ...
        if sort_by == "year":
            key_func = lambda x: getattr(x, sort_by)  # No need for .lower() for numeric values like year
        else:
        ...
        if self.sorted_by != sort_by:
            print(f"Sorting by {sort_by} using {algorithm}...")  # Debug message
            if algorithm == 'insertion':
                ...
                print("Invalid sorting algorithm.")  # For testing of earlier steps
                return

            # Update the sorted_by to reflect the current sorted state
